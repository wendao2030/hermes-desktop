...
Hermes Agent includes a built-in web interface for configuration management, session monitoring, and visual interaction.

#### Starting the Dashboard

```bash
# Start dashboard (default port 9119)
hermes dashboard
...
# Start with embedded TUI chat interface
hermes dashboard --tui

# Skip web UI build step (use existing dist)
...
2. **Session Management**
   ...
   - Resume sessions from web interface
   - View session transcripts

3. **Embedded TUI Chat**
   - Full Hermes TUI interface in browser
...
- **Default**: Binds to `127.0.0.1` only (localhost)
- **Insecure mode**: Use `--insecure` flag to bind to network (exposes API keys)
- **Session tokens**: Generated fresh on each server start
...
2. Open browser to: `http://localhost:9119`
3. For embedded TUI: `http://localhost:9119/chat`

#### Troubleshooting

- **Dashboard won't start**: Check if port is already in use
- **Build errors**: Use `--skip-build` if npm not available
- **No browser opens**: Use `--no-open` and manually open URL
- **Authentication fails**: Session token regenerated on each start

**See also**: `references/web-dashboard-discovery.md` for detailed discovery notes and verification steps.
